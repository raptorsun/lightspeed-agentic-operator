...
36a. **SA token mounting**: `PodSpecBuilder` MUST set `AutomountServiceAccountToken=true` for **execution** and **verification** steps (the agent needs the projected SA token to authenticate `oc`/`kubectl` commands against the cluster API using the per-proposal SA's RBAC). For **analysis** steps, `AutomountServiceAccountToken` MUST be `false` (analysis currently has no cluster access; read RBAC is tracked separately under OLS-3255).
