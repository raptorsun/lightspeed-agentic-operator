...
func TestIsTerminalPhase(t *testing.T) {
	tests := []struct {
		...
		{agenticv1alpha1.AgenticRunPhasePending, false},
		{agenticv1alpha1.AgenticRunPhaseAnalyzing, false},
		{agenticv1alpha1.AgenticRunPhaseExecuting, false},
		{agenticv1alpha1.AgenticRunPhaseVerifying, false},
	}
...
func TestPhaseColor(t *testing.T) {
	if c := PhaseColor(agenticv1alpha1.AgenticRunPhaseCompleted); c != ColorGreen {
		t.Errorf("expected green for Completed, got %q", c)
	}
	if c := PhaseColor(agenticv1alpha1.AgenticRunPhaseFailed); c != ColorRed {
		t.Errorf("expected red for Failed, got %q", c)
	}
	if c := PhaseColor(agenticv1alpha1.AgenticRunPhasePending); c != ColorReset {
		t.Errorf("expected reset for Pending, got %q", c)
	}
	if c := PhaseColor(agenticv1alpha1.AgenticRunPhaseEscalated); c != ColorMagenta {
		t.Errorf("expected magenta for Escalated, got %q", c)
	}
	if c := PhaseColor(agenticv1alpha1.AgenticRunPhaseEmergencyStopped); c != ColorMagenta {
		t.Errorf("expected magenta for EmergencyStopped, got %q", c)
	}
}

func TestColoredPhase(t *testing.T) {
	result := ColoredPhase(agenticv1alpha1.AgenticRunPhaseCompleted)
...
func TestSortAgenticRunsByAge(t *testing.T) {
	now := time.Now()
	items := []agenticv1alpha1.AgenticRun{
