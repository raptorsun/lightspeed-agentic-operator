...
func NewGetCmd(streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "get NAME",
		Short: "Display details of an AgenticRun",
		Example: `  # Show run details
  oc agentic run get fix-crash

  # Output as JSON
  oc agentic run get fix-crash -o json`,
...
func (o *GetOptions) printDetail(p *agenticv1alpha1.AgenticRun) {
	w := o.Out
	...
	fmt.Fprintf(w, "Analysis:          %s\n",
		stepStatusFromConditions(p.Status.Steps.Analysis.Conditions, agenticv1alpha1.AgenticRunConditionAnalyzed))
	...
	fmt.Fprintf(w, "Execution:         %s\n",
		stepStatusFromConditions(p.Status.Steps.Execution.Conditions, agenticv1alpha1.AgenticRunConditionExecuted))
	...
	fmt.Fprintf(w, "Verification:      %s\n",
		stepStatusFromConditions(p.Status.Steps.Verification.Conditions, agenticv1alpha1.AgenticRunConditionVerified))
	...
	if len(p.Status.Steps.Escalation.Results) > 0 || len(p.Status.Steps.Escalation.Conditions) > 0 {
		...
		fmt.Fprintf(w, "Escalation:        %s\n",
			stepStatusFromConditions(p.Status.Steps.Escalation.Conditions, agenticv1alpha1.AgenticRunConditionEscalated))
